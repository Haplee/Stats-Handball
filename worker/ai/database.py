...
def update_video_status(video_id, status):
    ...
    try:
        ...
        progreso = 5 if status == 'procesando' else (100 if status == 'completado' else 0)
        table = get_videos_table()
        stmt = update(table).where(table.c.id == video_id).values(status=status, progress=progreso)
...
def update_video_progress(video_id, progress):
    ...
    try:
        session.expire_all()
        table = get_videos_table()
        stmt = update(table).where(table.c.id == video_id).values(progress=progress)
        session.execute(stmt)
        session.commit()
    except Exception as e:
        session.rollback()
...
def update_video_results(video_id, results):
    ...
    try:
        session.expire_all()
        table = get_videos_table()
        stmt = update(table).where(table.c.id == video_id).values(results=results)
        session.execute(stmt)
        session.commit()
    except Exception as e:
        session.rollback()
