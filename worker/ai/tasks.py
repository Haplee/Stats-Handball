...
def calculate_pixel_distance(pasos):
    ...
    # np.hypot es más rápido y estable.
...
def analizar_video_partido(id_video, nombre_archivo=None, url_youtube=None):
    ...
    try:
        ...
        rutas_jugadores, clases_ids = seguidor_ia.trackear_partido(
            frames_generator
        )
        ...
        for id_jugador, pasos in rutas_jugadores.items():
            ...
            # Optimized: using vectorized calculation
            dist_pixeles = calculate_pixel_distance(pasos)
            dist_metros = dist_pixeles * 0.05
            velocidad_max = (
                (dist_metros / len(pasos)) * video_properties["fps"] * 3.6
            )
        ...
        total_seconds = (
            video_properties["frame_count"] / video_properties["fps"]
        )
        ...
        datos_finales = {
            ...
            "heatmap_all": [
                p for tray in trayectorias_por_jugador.values() for p in tray
            ][:300],
            "heatmap_players": trayectorias_por_jugador,
            "player_stats": sorted(
                stats_jugadores,
                key=lambda x: float(x['dist'].split()[0]),
                reverse=True
            )[:10]
        }
