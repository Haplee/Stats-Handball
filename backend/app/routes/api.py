...
def subir_video_partido():
    ...
    if archivo and es_formato_permitido(archivo.filename):
        ...
        nuevo_video = Video(nombre_fichero=nombre_limpio, ruta_fichero=ruta_guardado, estado='pending')
        db.session.add(nuevo_video)
        db.session.commit()

        # Mandamos el recado al worker de Celery para que empiece a currar
        current_app.celery.send_task('process_video_task', args=[nuevo_video.id, nombre_limpio])
...
def procesar_enlace_youtube():
    ...
    video_youtube = Video(enlace_youtube=url, nombre_fichero=f"YouTube: {url[:30]}...", estado='pending')
    db.session.add(video_youtube)
    db.session.commit()

    # Le pasamos la pelota al worker
    current_app.celery.send_task('process_video_task', args=[video_youtube.id, None, url])
